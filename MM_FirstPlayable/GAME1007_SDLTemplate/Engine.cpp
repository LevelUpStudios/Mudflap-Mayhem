...
int Engine::Init(const char* title, int xPos, int yPos, int width, int height, int flags)
{
	...
	if (SDL_Init(SDL_INIT_EVERYTHING) == 0) // If initialization is okay...
	{
		...
		if (m_pWindow != nullptr)
		{
			...
			if (m_pRenderer != nullptr)
			{
				...
				{
					m_pTexture = IMG_LoadTexture(m_pRenderer, "img/Truck.png");
					m_pBGtexture = IMG_LoadTexture(m_pRenderer, "img/bg3.png");
					m_pEtexture = IMG_LoadTexture(m_pRenderer, "img/ghost.png");
					m_pBtexture = IMG_LoadTexture(m_pRenderer, "img/Missile.png");
	...
	m_player.SetRekts( {0,0,201,98}, {50,350,126,72} ); // First {} is src rectangle, and second {} destination rect
	m_bg1.SetRekts( {0,0,570,1026}, {0,0,1920,640} );
